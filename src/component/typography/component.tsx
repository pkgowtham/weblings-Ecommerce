...
const calculateClamp = (
  ...
  return `clamp(${minSize}px, ${slopeVW.toFixed(2)}vw + ${yIntersection.toFixed(
    2
  )}px, ${maxSize}px)`;
};
...
const createStylesFromJson = (allStyles: {
  ...
  allStyles.desktop.forEach((desktopStyle) => {
    const tabletStyle = allStyles.tablet.find(
      (s) => s.name === desktopStyle.name
    );
    const mobileStyle = allStyles.mobile.find(
      (s) => s.name === desktopStyle.name
    );
